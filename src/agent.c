...
void agent_add_assistant_message(agent_t *a, const char *content,
                                 const tool_call_t *tool_calls, int tc_count) {
    cJSON *msg = cJSON_CreateObject();
    ...
    if (tool_calls && tc_count > 0) {
        ...
        for (int i = 0; i < tc_count; i++) {
            ...
            cJSON_AddStringToObject(fn, "arguments",
                                    tool_calls[i].raw_args ? tool_calls[i].raw_args : "{}");
...
void agent_add_tool_result(agent_t *a, const char *tool_call_id,
                           const char *content) {
    cJSON *msg = cJSON_CreateObject();
    ...
    for (int i = 0; i < a->pending_count; i++) {
        if (strcmp(a->pending[i].id, tool_call_id) == 0) {
            ...
            memmove(&a->pending[i], &a->pending[i + 1],
                    (size_t)(a->pending_count - i - 1) * sizeof(tool_call_t));
            a->pending_count--;
            break;
        }
    }
    ...
    if (d.tool_call_index >= 0) {
        ...
        while (ctx->raw_tc_count <= idx) {
            if (ctx->raw_tc_count >= ctx->raw_tc_cap) {
                ...
                ctx->raw_tcs = realloc(ctx->raw_tcs,
                                       (size_t)ctx->raw_tc_cap * sizeof(raw_tc_t));
            }
...
int agent_send(agent_t *a, const char *prompt, chunk_fn on_chunk,
               void *on_chunk_data, agent_response_t *out) {
    ...
    if (tc_count > 0) {
        agent_add_assistant_message(a, (text && text[0]) ? text : NULL,
                                    tcs, tc_count);
        /* Set pending */
        tool_calls_free(a->pending, a->pending_count);
